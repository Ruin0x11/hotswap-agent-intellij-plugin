...
public enum ConfigurationInfo {
    ...
    private boolean isDCEInstalledFromInstaller(Path dir, boolean altjvm) {
        Path jreDir;
        if (isJDK(dir)) {
    ...
    private boolean isDCEInstalledFromArchive(Path dir) {
        Path releaseFileDir = dir.resolve("release");

        if (!Files.exists(releaseFileDir)) {
            return false;
        }

        Properties prop = new Properties();
        try (final InputStream is = Files.newInputStream(releaseFileDir)) {
            prop.load(is);
        }
        catch (Exception e) {
            throw new IllegalStateException("Failed to read release file: " + e.getMessage() + ".");
        }

        return prop.getProperty("JVM_VARIANT").equals("\"Dcevm\"");
    }

    public boolean isDCEInstalled(Path dir, boolean altjvm) {
        if (altjvm) {
            return isDCEInstalledFromInstaller(dir, false);
        } else {
            return isDCEInstalledFromInstaller(dir, true) || isDCEInstalledFromArchive(dir);
        }
    }
